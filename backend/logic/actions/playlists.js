...
export default {
	 ...
	 * @param {Function} cb - gets called with the result
	 */
	deleteOrphanedArtistPlaylists: useHasPermission("playlists.deleteOrphaned", async function index(session, cb) {
		async.waterfall(
			[
				next => {
	 ...
	 * @param {Function} cb - gets called with the result
	 */
	createMissingArtistPlaylists: useHasPermission("playlists.createMissing", async function index(session, cb) {
		this.keepLongJob();
		this.publishProgress({
			status: "started",
